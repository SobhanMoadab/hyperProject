...
import { IProblemRepository, ResponseType } from "../../repos/IProblemRepository";
import { DefineProblemDTO } from "./DefineProblemDTO";
import { DuplicateProblemError } from "./DefineProblemErrors";

type Response = Either<DuplicateProblemError | UnexpectedError, Result<ResponseType>>;
...
export default class DefineProblem {
	...
	async execute(req: DefineProblemDTO): Promise<Response> {
		...
		try {
			const result = await this.problemRepo.save(dto);
